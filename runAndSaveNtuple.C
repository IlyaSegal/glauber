...
#include <TSystem.h>
#include <TVector3.h>
#include "readNucleonsPositions.h"
#ifdef HAVE_MATHMORE
...
TVector3 &TGlauNucleus::ThrowNucleons(Double_t xshift)
{
 ...
 cmscheck: /* start over here in case shift was too large */
  ...
  if (ifDCMSMM) 
  {
      vector<TLorentzVector> NucleonsPositions = readNucleonsPositions(inFileName, nEvents, idEvent, fN);
      for (Int_t i = 0; i<fN; ++i) {
        TGlauNucleon *nucleon=(TGlauNucleon*)(fNucleons->At(i));
        if (xshift<0) nucleon->SetXYZ(NucleonsPositions.at(i).Px(),NucleonsPositions.at(i).Py(),NucleonsPositions.at(i).Pz());
        else nucleon->SetXYZ(NucleonsPositions.at(i+fN).Px(),NucleonsPositions.at(i+fN).Py(),NucleonsPositions.at(i+fN).Pz());
        }
  }
...
Bool_t TGlauberMC::NextEvent(Double_t bgen)
{
  ...
  if (ifDCMSMM) 
  {
      vector<TLorentzVector> ImpactParameter = readNucleonsPositions(inFileName, nEvents, idEvent, fAN, true);
      bgen = (ImpactParameter.at(0)).P();
  }
